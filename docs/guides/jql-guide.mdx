...
<Note>
If the server is configured with a
[read boundary](/docs/advanced/jira-scope) (`JIRA_JQL_FILTER` or
`JIRA_PROJECTS_FILTER`), it is ANDed into every query you send. Results are
always a subset of that boundary — an `OR` in your query cannot widen it — so a
query that returns nothing may simply fall outside it.
</Note>
