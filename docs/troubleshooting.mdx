...
<AccordionGroup>
  ...
  <Accordion title='"Outside the readable scope" / "outside the writable scope"'>
    These messages come from the server's own
    [Jira read/write scope](/docs/advanced/jira-scope), not from Jira:
    ...
    The issue did not match `JIRA_JQL_FILTER` / `JIRA_WRITE_JQL_FILTER`, did not
    match `JIRA_PROJECTS_FILTER`, does not exist, or is invisible to the
    configured credentials — the check is fail-closed, so all four look alike.

    A different message — "this server's access scope could not be evaluated" —
    means the boundary JQL itself was rejected by Jira. Fix the filter; the
    agent cannot work around it.

    Diagnose without editing the configuration:

    ```bash
    uvx mcp-atlassian --jira-scope-check PROJ-123
    ```

    It prints the boundaries in effect, the query a search turns into, and a
    `writable` / `read-only` / `denied` verdict for each key you pass. Searches
    that silently return nothing have the same cause: the read boundary is
