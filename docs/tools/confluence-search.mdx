...
| `spaces_filter` | `string` | No | (Optional) Comma-separated list of space keys to filter results by. Narrows within the server's configured CONFLUENCE_SPACES_FILTER; it cannot widen it or reach spaces the server excludes. Use empty string to apply no additional narrowing. |
**Example:**
