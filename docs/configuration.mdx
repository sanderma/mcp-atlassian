...
| `JIRA_JQL_FILTER` | Limit what Jira issues are visible at all (see [Jira Read/Write Scope](#jira-read-write-scope)) | `project = PROJ AND labels = agent` |
| `JIRA_WRITE_JQL_FILTER` | Limit which visible Jira issues may be modified | `team = ours` |
...
### Jira Read/Write Scope

`READ_ONLY_MODE` is all-or-nothing. Two JQL boundaries let you split it per
issue instead — the agent reads broadly but only changes what you allow:

| Variable | Boundary | Effect |
|----------|----------|--------|
| `JIRA_JQL_FILTER` | Read | ANDed into every JQL query the server issues, and required of any issue addressed directly by key. Issues that do not match are invisible. |
...
Read the whole instance, write only your own team's issues:

```bash
JIRA_WRITE_JQL_FILTER=team = ours
```

Restrict visibility too — see one project, write only your team's issues in it:

```bash
JIRA_JQL_FILTER=project = PROJ
JIRA_WRITE_JQL_FILTER=project = PROJ AND team = ours
```

Both are also available as `--jira-jql-filter` / `--jira-write-jql-filter`.

**Semantics**

- **Configuration-only.** No tool argument can widen a boundary; a caller may
  only narrow further, as with `JIRA_PROJECTS_FILTER`.
- **Fail-closed.** An issue that does not come back from the scope query — it
  does not match, does not exist, or is invisible to the credentials — is
  refused. A failed scope lookup denies rather than allows.
- **Composable.** `JIRA_PROJECTS_FILTER` still applies and is checked first,
  without an API call (project prefixes are compared locally).
- **Cost.** When a JQL boundary is set, a tool addressing issues by key runs
  one extra JQL search to validate them (batched for multi-issue calls). With
  no boundary configured the check is skipped entirely.

**What it does not cover**

Issue *creation* has no existing issue to match, so `JIRA_WRITE_JQL_FILTER`
cannot constrain it — bound creation with `JIRA_PROJECTS_FILTER` /
`JIRA_JQL_FILTER` (which do apply at project level), or disable the create
tools via `ENABLED_TOOLS` / `TOOLSETS`. Boundaries apply to Jira issues; they
do not filter Confluence, sprint/board/version metadata, or user lookups.

These are server-side guardrails against agent mistakes, layered on top of —
not a replacement for — Jira permissions on the account the server uses.
