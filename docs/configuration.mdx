...
| `JIRA_JQL_FILTER` | Limit what Jira issues are visible at all (see [Jira Read/Write Scope](#jira-read-write-scope)) | `project = PROJ AND labels = agent-visible` |
| `JIRA_WRITE_JQL_FILTER` | Limit which visible Jira issues may be modified | `labels = automation` |
...
Read the whole instance, change only the issues labelled for the agent:

```bash
JIRA_WRITE_JQL_FILTER=labels = agent-writable
```

Restrict visibility too — see one project, change only its automation issues:
...
Preflight a configuration before handing it to an agent — this reports what each
boundary allows and exits non-zero if Jira rejects a filter:
...
- **Not a creation limit.** No boundary constrains `jira_create_issue` or
  `jira_batch_create_issues`; drop those tools with `ENABLED_TOOLS` / `TOOLSETS`
  instead.
- **Jira issues only.** Confluence, board/sprint/version metadata and user
...
See [Jira Read/Write Scope](/docs/advanced/jira-scope) for the full semantics,
enforcement details, and how to hand a narrower configuration to a subagent.
...
```bash
# Restrict to core tools only (~36 tools across 6 core toolsets)
TOOLSETS=default
