...
class TestMarkdownToJiraParser:
    ...
    def test_mid_word_characters_not_escaped(self, preprocessor):
        """Boundary-only effect chars stay readable between word chars."""
        markdown = "Check well-known 2024-01-15 values in C-suite docs."
        result = preprocessor.markdown_to_jira(markdown)
        assert "well-known" in result
        assert "2024-01-15" in result
        assert "\\" not in result

    def test_intraword_underscores_escaped(self, preprocessor):
        """Jira italicizes intraword _..._, so underscores are escaped."""
